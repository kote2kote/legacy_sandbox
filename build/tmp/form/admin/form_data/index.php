...
if($sql_result_field_list[$num]=="id"){
	$id=$sql_result_item[$i][$num];
	echo $id;
} else if ($sql_result_field_list[$num]=="faq_category"){
...
echo($row2["shurui"]);
		...
		$sql_result_field_list[$num]=="name" ||
		$sql_result_field_list[$num]=="email" ||
		$sql_result_field_list[$num]=="adress2" ||
		$sql_result_field_list[$num]=="adress3" ||
		$sql_result_field_list[$num]=="faq_naiyou")
{
	
//検索用語が含まれる文字列のフォントを赤で表示されるようhtmlタグ付きに変換
$sql_result_item[$i][$num]   = str_replace($key_word,"<span class=\"red\">".$key_word."</span>",$sql_result_item[$i][$num]);
echo $sql_result_item[$i][$num];
